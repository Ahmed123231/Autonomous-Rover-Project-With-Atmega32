...
int main(void)
{
			...
			// Update sensor readings and display the current status
			...
			ROVER_LCD_PrintStatus(&Rover_state); // Display rover status
				...
				else
				{
					...
					if ((Rover_state.F_Obj != 1 || Rover_state.Distance < 10) && Rover_state.B_Obj != 1)
					{
						...
						if (Rover_state.L_Obj == 1)
						{
							...
							if (Rover_state.F_Obj == 1 || Rover_state.Distance > 10)
							{
								Rover_voidMOVFWD(50); // Move forward
								Buzzer_voidToggle();
								ROVER_LCD_PrintStatus(&Rover_state);
								_delay_ms(50);
							}
							...
							{
								Rover_voidMOVBCWD(50); // Move backward
								Buzzer_voidToggle();
								ROVER_LCD_PrintStatus(&Rover_state);
								_delay_ms(50);
							}
